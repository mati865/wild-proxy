...
// - Implement the proper solution and use fallback as a fallback
// - Preserve colours for errors

/// Fallback and ask the OG linker driver if we cannot figure it out ourselves
pub fn fallback() -> Result<()> {
    ...
    let build_and_assemble_commands = if piped {
        shell_cmds = Vec::with_capacity(commands.build_and_assemble.len());
        let mut commands_iter = commands.build_and_assemble.into_iter();
        while let Some(cmd) = commands_iter.next() {
            if cmd.ends_with("|") {
                shell_cmds.push([cmd, " ", commands_iter.next().unwrap()].concat());
            } else {
                shell_cmds.push(cmd.to_string())
            }
        }
...
fn parse_gcc(dumped_lines: Lines) -> Result<Commands> {
    ...
    let linker_command = commands.pop_if(|command| {
        let path = Path::new(command.split(' ').next().unwrap());
        // Collect2 binary is responsible for linking; other binaries compile or assemble
...
#[cfg(test)]
mod test {
    use crate::fallback::{Commands, obtain_whole_command};
...
Supported LTO compression algorithms: zlib zstd
gcc version 15.2.1 20260103 (GCC)
COLLECT_GCC_OPTIONS='-pipe' '-M' '-mtune=generic' '-march=x86-64' '-dumpdir' 'a-'
 /usr/lib/gcc/x86_64-pc-linux-gnu/15.2.1/cc1 -E -quiet -M test.c "-mtune=generic" "-march=x86-64" -dumpdir a- -dumpbase test.c -dumpbase-ext .c
COMPILER_PATH=/usr/lib/gcc/x86_64-pc-linux-gnu/15.2.1/:/usr/lib/gcc/x86_64-pc-linux-gnu/15.2.1/:/usr/lib/gcc/x86_64-pc-linux-gnu/:/usr/lib/gcc/x86_64-pc-linux-gnu/15.2.1/:/usr/lib/gcc/x86_64-pc-linux-gnu/
LIBRARY_PATH=/usr/lib/gcc/x86_64-pc-linux-gnu/15.2.1/:/usr/lib/gcc/x86_64-pc-linux-gnu/15.2.1/../../../../lib/:/lib/../lib/:/usr/lib/../lib/:/usr/lib/gcc/x86_64-pc-linux-gnu/15.2.1/../../../:/lib/:/usr/lib/
COLLECT_GCC_OPTIONS='-pipe' '-M' '-mtune=generic' '-march=x86-64' '-dumpdir' 'a.'
"#;
        ...
        let expected = Commands {
            build_and_assemble: vec![
                r#"/usr/lib/gcc/x86_64-pc-linux-gnu/15.2.1/cc1 -E -quiet -M test.c "-mtune=generic" "-march=x86-64" -dumpdir a- -dumpbase test.c -dumpbase-ext .c"#,
            ],
            link: None,
        };
        assert_eq!(expected, obtain_whole_command(input.lines()).unwrap());
